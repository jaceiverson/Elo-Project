...
def gg_league(save=True,new_run = False,sheet_name= 'TASK REVIEW'):
    #1
    google_sheets_in = connect('TASK REVIEW2').worksheet('title','Indv League')
    google_sheets_out = connect('TASK REVIEW2')
    ...
    #7
    if new_run:
        gg = generic_league(df,
                            'Total Score','./GG/fresh-league-{}.p'.format(dt.date.today().strftime('%Y-%m-%d')),
                            save=save,
                            lsw=False
                            )
    else:
